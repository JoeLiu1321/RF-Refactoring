...
from python_package.rfrefactoring.usageFinder import *
...
class KeywordUsageFinderTest(unittest.TestCase):
    def setUp(self):
        ...
        self.finder = KeywordUsageFinder()
    ...
    def test_find_kw_usage_from_keyword(self):        
        ...
        usages = self.finder.find_usage_from_keyword('Log',source_keyword)
        self.assertEqual(len(usages),3)
    
    def test_find_kw_usage_from_setting(self):
        setting_table = self.suite.setting_table
        usages = self.finder.find_usage_from_settings('Login EzScrum',setting_table)
    ...
    def test_find_kw_usage_from_testcase(self):
        ...
        usages = self.finder.find_usage_from_testcase('123', testcase)
        self.assertEqual(1, len(usages))

    def test_find_kw_usage_from_testdataFile(self):
        usages = self.finder.find_usage_from_testdataFile('Input Field',self.suite)
    ...
    def test_find_kw_usage_from_resource(self):
        ...
        usages = self.finder.find_usage_from_testdataFile('Find Usage Test Keyword1',ezScrum_resource)
        self.assertEqual(ezScrum_resource, usages['testdata'])
        self.assertEqual(3, len(usages['references']))

class VariableUsageFinderTest(unittest.TestCase):
    def setUp(self):
        self.variable_finder = VariableUsageFinder()
    ...
    def test_find_glob_var_usage_from_keyword(self):
        def get_variable_usage_from_keyword(variableName, keywordName):
            ...
            return self.variable_finder.find_global_variable_usage_from_keyword(variableName, keyword)
    ...
    def test_find_glob_var_usage_in_testcase(self):
        def get_variable_usage_from_testcase(variableName, testcaseName):
            ...
            self.assertIsNotNone(testcase)
            return self.variable_finder.find_global_variable_from_testcase_obj(variableName, testcase)
    ...
    def test_find_glob_var_usage_from_setting_table(self):
        def get_usage_from_setting_table(variable, table):
            return self.variable_finder.find_global_variable_usage_from_setting(variable, table)
    ...
    def test_find_glob_var_usage_from_testdata_file(self):
        def test_with_testcase():
            ...
            usages = self.variable_finder.find_global_variable_from_testdata_file(variable, self.suite)
        ...
        def test_with_resource_file():
            ...
            self.assertIsNotNone(variable)
            usages = self.variable_finder.find_global_variable_from_testdata_file(variable, resource_file)
